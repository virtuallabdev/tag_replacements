...
fn save_file(filenane: &str, folder: &str, content: &str) -> std::io::Result<()> {
    ...
    let base_path = "C:/Lavori/Repos/tag_replacements/test_files";
...
pub fn convert(path: &str) -> std::io::Result<()> {
    ...
    save_file("CMS-TEXTE-IT-20230922-145744.XML.txt", "converted",  &processed_content).unwrap();
    save_file("CMS-TEXTE-IT-20230922-145744.XML.json", "tmp", &serialized_data).unwrap();
...
pub fn revert(path: &str) -> std::io::Result<()> {
    let mut file_content = String::new();
    let mut tag_vec = Vec::new();
    let placeholder: &str = " § ";
